...
class TrainGan(TrainBaseClass):
    ...
    def train(self,
        ...
        if use_pretrained:
            ...
            self.evaluation_d(self.data.test_triples_for_classification, 'test(d)', 0)
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for batch_true, batch_fake in self.data.get_batch(batch_size):
                ...
                if g_use_negative:
                    _, _, tail_embedding_fake_hard = self.embedding.forward(batch_fake[:, 0],
                                                                            batch_fake[:, 1],
                                                                            e2=batch_fake[:, 2])
                    batch_true_loss = torch.sum((head_embedding + relation_embedding - tail_embedding)**2,dim=1)
                    batch_fake_loss = torch.sum((head_embedding + relation_embedding - tail_embedding_fake_hard)**2,dim=1)
            ...
            if epoch != 0 and epoch % evaluation_times == 0:
                with torch.no_grad():
                    ...
                    self.evaluation_d(self.data.test_triples_with_reverse, "test(d)",epoch)
        ...
        with torch.no_grad():
            ...
            self.print_file.write('\nepoch: {0}. TransE score: \n'.format(epochs))
            self.evaluation_d(self.data.test_triples_with_reverse, "test(d)", epochs)
    ...
    def evaluation_d(self, eval_dataset, dataset_name, epoch):
        ...
        for tail, reverse_relation in zip(tail_list, reverse_relation_list):
            ...
            head_embeddings = self.embedding.get_evaluation_weight(reverse_relation)
            tail_embedding = tail_embedding.expand(head_embeddings.size()[0], head_embeddings.size()[1])
            relation_reverse_embedding = relation_reverse_embedding.expand(head_embeddings.size()[0],
                                                                           head_embedding.size()[1])
            pred_head = self.discriminator.forward(tail_embedding, relation_reverse_embedding, head_embeddings)
            max_values, argsort_head = torch.sort(pred_head, 0, descending=False)
            argsort_head_list.append(np.expand_dims(argsort_head.cpu().numpy(), axis=0))
        argsort_head_list = np.concatenate(argsort_head_list, axis=0)

        MR, hit10, MRR = ranking_and_hits(dataset_name, head_relation_tail_data, tail_reverse_relation_head_data,
                                          argsort_tail_list,
                                          argsort_head_list, print_file=self.print_file)

        if epoch != 0 and epoch % 100 == 0:
            ranking_and_hits_filter(dataset_name, head_relation_tail_data, tail_reverse_relation_head_data,
                                    argsort_tail_list, argsort_head_list, self.data.gold_triples_dict,
